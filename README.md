...
- **變數替換**: 使用 `{{variable_name}}` 語法，套用於 URL、Headers、Body
- **基礎 URL 自動套用**: 選擇環境後可直接輸入相對路徑，系統會解析為完整 URL
- **預設標頭**: 每個環境可設定預設 Headers
- **快速套用與預覽**: 可在管理視窗預覽環境內容，並一鍵設為目前環境
- **目前環境持久化**: 重新開啟程式後會保留上次使用的環境
...
在 URL、Headers 或 Body 中使用 `{{variable}}` 語法：
...
1. 點擊「🌐 管理環境」按鈕
2. 點擊「➕ 新增環境」
3. 填入環境名稱、基礎 URL、認證資訊、預設 Headers 與環境變數
4. 點擊「儲存」

### 編輯既有環境

1. 點擊「🌐 管理環境」按鈕
2. 選取既有環境後點擊「✏ 編輯環境」，或直接雙擊列表項目
3. 更新 Base URL、Headers、變數或認證設定後儲存

### 切換環境

使用工具列上的環境下拉選單快速切換；選取後會更新目前環境摘要，並可直接輸入 `/v1/users` 這類相對路徑。

### 管理視窗操作

環境管理視窗支援：
- 新增、編輯、刪除環境
- 預覽 Base URL、認證、Headers、變數
- 一鍵設為目前環境
